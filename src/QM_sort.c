...
// constants
#define     MERGE       1023    // N <= MERGE : Indirect Mergesort
...
static void hqsort(void *base, size_t nmemb) {
    ...
    if (nmemb > 1) {
        ...
        for (array = first, index = dst; array < last; array += length, index++) {
            ...
            char *p2, *p3;
            if (*(p1 = index) != array) {
                p2 = array; copy(pivot, p2);     // save an element
                do {
                    p3 = *p1; copy(p2, p3);     // copy an element
                    *p1 = p2;               // restore into dst[]
                    p1 = &dst[((p2 = p3) - first) / length];
                } while (*p1 != array);
                *p1 = p3; copy(*p1, pivot);       // restore saved element
            }
        }
    }
}
