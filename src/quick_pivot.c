...
static size_t mid1 = 31, rnd1 = 0, med3 = 127, med5 = 4095, medL = 0; // Threshold to change a choosing pivot
...
#define first (char *)base
        ...
        else if (nmemb <= rnd1) {  // choose a random element
            hole = first + (nmemb * rand() / RAND_BASE) * length;
#ifdef  DEBUG
            if (trace_level >= TRACE_DUMP)
                fprintf(OUT, "Pivot is random in %s elements : %s at %ld\n",
                        dump_size_t(NULL, nmemb), dump_data(hole), (hole - first) / length);
#endif
        }
...
void quick_pivot(void *base, size_t nmemb, size_t size, int (*compare)(const void *, const void *))
{
    ...
    if (random1) rnd1 = random1;
    if (median3) med3 = median3;
    if (median5) med5 = median5;
    if (medianL) medL = medianL;
    sort(base, nmemb);
}
